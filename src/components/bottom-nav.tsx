...
export function BottomNav({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: (tab: string) => void }) {
  return (
    <nav className={cn(
      ...
      "md:bottom-6 md:left-1/2 md:-translate-x-1/2 md:w-auto md:h-auto md:rounded-full md:border md:bg-white/20 md:dark:bg-zinc-800/20 md:backdrop-blur-lg md:shadow-lg md:px-4 md:py-2 md:dark:border-white/30"
    )}>
        ...
        {navItems.map((item) => (
          <button
            ...
            className={cn(
              ...
              activeTab === item.name
                ? 'text-primary md:text-white md:bg-primary/80'
                : 'text-muted-foreground hover:text-primary md:text-neutral-200 md:hover:bg-white/10'
            )}
          >
