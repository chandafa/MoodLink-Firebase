...
import { ArrowLeft, ArrowRight, BookText, FilePlus, Search } from 'lucide-react';
...
function JournalEntryCard({ entry, onSelect }: { entry: JournalEntry; onSelect: () => void }) {
  ...
  return (
    ...
    >
        <Card onClick={onSelect} className="cursor-pointer h-full flex flex-col hover:border-primary transition-colors duration-200">
            <CardHeader>
                ...
                <AnimatePresence>
                    ...
                    >
                        {paginatedEntries.map(entry => (
                            <JournalEntryCard key={entry.id} entry={entry} onSelect={() => onSelectEntry(entry.id)} />
                        ))}
                    </motion.div>
                </AnimatePresence>
                {totalPages > 1 && (
