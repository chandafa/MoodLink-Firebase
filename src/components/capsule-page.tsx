...
import { Hourglass, Lock, Unlock, FilePlus } from 'lucide-react';
...
export function CapsulePage({ onSelectEntry }: { onSelectEntry: (id: string | null) => void; }) {
  const { entries, isLoaded } = useJournal();
  const now = useMemo(() => new Date(), []);

  const { lockedCapsules, unlockedCapsules } = useMemo(() => {
    const capsules = entries
      .filter(entry => entry.postType === 'capsule')
